...
(function () {
  ...
  (function pinSections() {
    ...
    /* Три условия, и все обязательны.
       ...
       Второе — она должна занимать почти весь экран. Короткая прилипшая
       секция оставляет под собой полосу следующей, и наезд одной на другую
       перестаёт читаться: видно просто застывший текст.

       Третье — следующая секция должна быть другого цвета. Приём читается
    ...
    function apply() {
      ...
      secs.forEach(function (s, i) {
        ...
        var fills = h <= vh + 1 && h >= vh * 0.92;
        s.classList.toggle('is-pinned', fills && bg(s) !== bg(next));
      });
    }
    apply();
