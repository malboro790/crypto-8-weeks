...
(function () {
  ...
  (function pinSections() {
    ...
    /* Соседом считается следующая ВИДИМАЯ секция.

       Скрытые секции остаются в разметке — их прячут, а не удаляют, чтобы
       вернуть одним словом. Но у скрытого элемента прямоугольник нулевой,
       и проверка «следующая закрыла верх окна» на нём истинна всегда: секция
       перед скрытой оказывалась отпущенной навсегда и не прилипала ни разу.
       По той же причине с ней сравнивался и цвет фона — у скрытого он свой
       и к передаче цвета отношения не имеет.

       Список пересобирается на каждом пересчёте: видимость меняется не только
       при вёрстке, но и медиазапросом. */
    var live = secs;
    function relist() {
      live = secs.filter(function (s) { return getComputedStyle(s).display !== 'none'; });
      secs.forEach(function (s) {
        if (live.indexOf(s) !== -1) return;
        s.__pinnable = false;
        s.classList.remove('is-pinned', 'is-released');
        s.style.top = '';
      });
    }
    ...
    function apply() {
      ...
      live.forEach(function (s, i) {
        var next = live[i + 1] || document.querySelector('.footer');
        /* Дробная высота, а не offsetHeight: тот округляет до целого, и от
           этого числа зависит само решение прилипать. */
        var h = s.getBoundingClientRect().height;
        var tall = h > vh;
        var pin = bg(s) !== bg(next) && (tall ? long : h >= vh * 0.92);
        /* apply() решает только, ИМЕЕТ ЛИ секция право прилипать. Где именно
           она замрёт, скрипт больше не считает: это делает CSS нижним краем,
           и там нечему устаревать при изменении высоты. */
        s.__pinnable = pin;
