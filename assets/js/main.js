...
(function () {
  ...
  (function pinSections() {
    ...
    function apply() {
      ...
      /* Пишем только при изменении: запись меняет min-height прилипших секций,
         а значит будит наблюдателя за размером, и тот зовёт apply() снова.
         Величина от этого не меняется, круг замыкается сам, но каждый холостой
         пересчёт стоит работы на ровном месте. */
      if (vh !== lastVh) {
        lastVh = vh;
    ...
    /* Ресайз считаем сразу, отложенный вызов добивает хвост жеста.

       Одна задержка на 120 мс означала, что всё перетаскивание окна секция
       стоит по старой высоте окна: смещение устарело ровно так же, как при
       изменении высоты самой секции. */
    var t = null;
    window.addEventListener('resize', function () {
      apply();
      clearTimeout(t);
    ...
    if (window.ResizeObserver) {
      /* Считаем СИНХРОННО, прямо в вызове наблюдателя, а не через кадр.

         Здесь и была причина редких срабатываний. Раньше вызов лишь заказывал
         пересчёт на следующий кадр. Но браузер зовёт наблюдателя уже после
         раскладки и ещё ДО отрисовки текущего кадра — значит кадр, в котором
         высота изменилась, успевал нарисоваться со старым смещением.

         Опасен только один случай из двух. Если секция подросла, старое
         смещение уводит её нижний край ниже сгиба — прореха невозможна.
         А если ужалась, край не доходит до сгиба ровно на величину усадки,
         и на один кадр под секцией видно то, что лежит ниже. При быстрой
         прокрутке этот единственный кадр и читается как вспышка по низу
         экрана. Поводов для усадки немного — поздняя загрузка снимка,
         подстановка шрифта, смена медиазапроса, — оттого и «редко».

         Считая здесь, попадаем в тот же кадр, что и изменение высоты:
         устаревшему смещению негде показаться. Зациклиться нельзя: apply()
         правит top, который размеров не меняет, высоту окна пишет только при
         её изменении, а переключение класса меняет поток в ту же сторону,
         в какую сделан вывод, — состояние сходится, а не качается. */
